    ...
    <div className="resultBoard">
      ...
      {leads.length > 0 && (
        <div className="leadPipeline">
          {leads.slice(0, 8).map((lead, index) => (
            <div className="leadCard" key={lead.lead_id || index}>
              <div className="leadTop">
                <b>{lead.priority || '线索'} / score {lead.lead_score || lead.score || '-'}</b>
                <span>{lead.priority === 'A' ? '人工上报' : '自动观察'}</span>
              </div>
              <p>{lead.original_text || lead.text}</p>
              <div className="leadStatusRow">
                <em>状态：{lead.capture_angle || lead.buyer_stage || '待判断'}</em>
                <em>动作：{lead.priority === 'A' ? '人工确认后回复/私域承接' : '先沉淀为选题素材'}</em>
              </div>
              {(lead.public_reply || lead.reply_draft) && <p className="replyBox">回复草稿：{lead.public_reply || lead.reply_draft}</p>}
              {lead.follow_up_question && <p className="warnBox">筛选追问：{lead.follow_up_question}</p>}
              {lead.script_hook && <p className="hookBox">可转视频：{lead.script_hook}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {insights.length > 0 && (
        <div className="productCardGrid">
          {insights.slice(0, 6).map((item: any, index: number) => (
            <div className="productCard" key={index}>
              <h3>内容结构 #{index + 1}</h3>
              <p>{item.title || item.topic_angle || '未命名内容'}</p>
              {item.opening_hook && <p className="hookBox">开头结构：{item.opening_hook}</p>}
              {item.timeline_text && <p>{item.timeline_text}</p>}
              {item.shot_hint && <p className="warnBox">镜头建议：{item.shot_hint}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {(script || timeline) && (
        <div className="productCard wide">
          ...
          {script?.script_hook && <p className="hookBox">Hook：{script.script_hook}</p>}
          {script?.script_text && <p>{script.script_text}</p>}
          {timeline?.srt_preview && <pre className="miniPre">{timeline.srt_preview}</pre>}
        </div>
      )}
...
export default function OpenClawWorkbench() {
  ...
  const [materialDuration, setMaterialDuration] = useState(8)
  ...
  const estimated = useMemo(() => {
    ...
    const material = Math.max(3, Number(materialDuration) || 8)
    const chars = Math.round(seconds * 4.2)
    const segments = Math.max(3, Math.min(16, Math.ceil(seconds / Math.min(5, material))))
    const clipCount = Math.max(1, Math.ceil(seconds / material))
    return { seconds, material, chars, segments, clipCount }
  ...
  async function runAction(action: string) {
    ...
    try {
      ...
      if (action === 'comment-adapter') {
        data = await apiPost('/api/video/openclaw/comments/analyze', {
        ...
        data = await apiPost('/api/video/openclaw/llm-enhance/comments', {
          raw_export: rawExport,
          campaign_context: campaignContext,
          min_score: 40,
          max_llm_items: realDeepSeek ? 20 : 8,
          dry_run: !realDeepSeek,
          save_rule_leads: true,
        })
      } else if (action === 'content-intel') {
        data = await apiPost('/api/video/openclaw/content/analyze', {
          raw_export: rawExport,
          campaign_context: campaignContext,
          save: true,
          max_items: 300,
        ...
        data = await apiPost('/api/video/openclaw/timeline/plan', {
          ...
          max_items: 300,
          render_hint: {
            material_duration_seconds: materialDuration,
            estimated_clip_count: estimated.clipCount,
            edit_strategy: '按素材时长切段，评论强意图段加快节奏，风险/信任段放慢节奏，口播优先对齐素材。',
          },
        ...
        const prompt = result?.script?.script_text || result?.script?.script_hook || rawExport.slice(0, 1000)
        data = await apiPost('/api/video/full-ai/start', {
          prompt,
          market,
          platform,
          target_duration: targetDuration,
          max_shots: Math.min(3, estimated.clipCount),
  ...
  return (
    <section className="workspacePanel">
      <div className="panelHero greenHero">
        <div>
          <p>OPENCLAW CAPTURE BOARD</p>
          <h2>OpenClaw 获客截流看板</h2>
          <span>这里看 OpenClaw 截到了什么流、哪些能回复、哪些要人工上报、哪些能转下一条视频。</span>
        </div>
        <b>不调用 fal.ai</b>
      </div>
      ...
      <div className="inputGrid four">
        ...
        <label>视频长度/秒<input type="number" value={targetDuration} onChange={(e) => setTargetDuration(Number(e.target.value || 28))} /></label>
        <label>单素材时长/秒<input type="number" value={materialDuration} onChange={(e) => setMaterialDuration(Number(e.target.value || 8))} /></label>
      </div>

      <div className="estimateStrip">
        <b>文案长度建议：约 {estimated.chars} 字 / {estimated.segments} 段口播</b>
        <span>素材预计切 {estimated.clipCount} 段，配音分段按素材时长自动分配。</span>
      ...
      <div className="buttonRow">
        <button onClick={() => runAction('comment-adapter')} disabled={!!busy}>分析截流评论</button>
        <button className="purple" onClick={() => runAction('comment-llm')} disabled={!!busy}>生成回复/上报建议</button>
        <button onClick={() => runAction('content-intel')} disabled={!!busy}>内容结构分析</button>
        <button className="green" onClick={() => runAction('timeline-plan')} disabled={!!busy}>生成 Timeline</button>
        <button className="red" onClick={() => runAction('generate-video')} disabled={!!busy}>直接生成视频</button>
      </div>
