...
const DEFAULT_CONFIG: StoredConfig = {
  ...
  subtitleStyle: 'dynamic_white_yellow', captionSize: 154, captionMotion: 'smart_mix', captionPosition: 'auto',
...
const FONT_SIZES = [{label:'标准大字',value:142},{label:'推荐',value:154},{label:'冲击大字',value:170},{label:'超大',value:188}]
...
function loadConfig(): StoredConfig {
  ...
  try {
    ...
    return {
      ...
      captionSize: Number(parsed.captionSize)<142?154:Math.max(142,Math.min(198,Number(parsed.captionSize)||154)),
...
export default function DynamicEditV2Selector({shotCount=0}:{shotCount?:number}) {
  ...
  return <section className="dynamic-edit-v2-shell v19" data-dynamic-edit-v2="true" data-placement="shot-plan-section">
    <div className="v19-topline">
      <div className="v19-title"><span>V10.40.8.34 · DEDUP KEYWORD + ENTITY LOCK + CTA</span><h4>AI 教学口播精剪</h4><p>关键词会从原字幕中抽离后再放大，杜绝上下重复两套字；办公、大学、租客来源、现有配套和 CTA 使用具体场景硬锁定。城市天际线最多约占 20%，结尾必须使用人物/手机/咨询互动并配评论气泡。</p></div>
    ...
    {config.engine==='dynamic_v2' && <div className="v19-workbench">
      ...
      {tab==='sound' && <div className="v19-compact-grid">
        ...
        <label className="v19-field"><span>动态标注位置</span><select value={config.stickerLayout} onChange={(e:ChangeEvent<HTMLSelectElement>)=>setConfig(c=>({...c,stickerLayout:e.target.value as DynamicStickerLayout}))}><option value="auto_safe">自动安全区</option><option value="top">上方两角</option><option value="side">左右侧边</option></select></label>
      </div>}

      {tab==='rules' && <div className="v19-rule-grid"><article><b>长句稳镜</b><span>同一意思不按逗号乱切；相邻镜头禁止复用同一素材或同类城市地标画面。</span></article><article><b>地点快切</b><span>咖啡厅、商场、学校等并列小场景按出现顺序逐项切换，不把普通长句拆碎。</span></article><article><b>素材记忆</b><span>记录素材、源片段、语义角色和速度；最近三条优先换新，高匹配旧素材允许复用。</span></article><article><b>慢镜加速</b><span>缓慢航拍、慢推和静态镜头自动使用约 1.10–1.20 倍速度；合同和文字特写保持原速。</span></article><article><b>连续配音</b><span>普通解释尽量一次连续合成，只在真实 CTA 或明确长停顿处分组，避免 0.4 秒以上硬拼接。</span></article><article><b>收尾结构</b><span>长结尾按风险提醒、互动问题、评论 CTA 拆成不同画面，禁止单一双子塔空镜拖满。</span></article><article><b>参考字效</b><span>主字幕默认不少于 142px；关键词会从原字幕抽离后单独弹到 154%，并配合圈选、箭头、下划线等动态标注。旧 Emoji、小线稿图标、大文本框和随机角标全部禁用。语义契合优先级高于素材新鲜度；城市地标设置 20% 配额，CTA 必须匹配人物/手机/咨询互动。</span></article></div>}
    </div>}
  </section>
}
