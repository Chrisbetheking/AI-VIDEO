...
export default function DynamicEditV2Selector({shotCount=0}:{shotCount?:number}) {
  ...
  return <section className="dynamic-edit-v2-shell v19" data-dynamic-edit-v2="true" data-placement="shot-plan-section">
    <div className="v19-topline">
      <div className="v19-title"><span>V10.40.8.31 · CLEAN TEXT + PRO STICKERS</span><h4>AI 教学口播精剪</h4><p>默认只做纯大字字幕和关键词放大，不再生成遮挡画面的文本框；咖啡厅、商场、学校等语义会触发专业小贴纸与可听音效。</p></div>
    ...
    {config.engine==='dynamic_v2' && <div className="v19-workbench">
      ...
      {tab==='sound' && <div className="v19-compact-grid">
        ...
        <label className="v19-field"><span>贴纸风格</span><select value={config.stickerStyle} onChange={(e:ChangeEvent<HTMLSelectElement>)=>setConfig(c=>({...c,stickerStyle:e.target.value as DynamicStickerStyle}))}><option value="icons">专业线稿贴纸（推荐）</option><option value="smart_mix">专业语义贴纸</option><option value="doodles">轻量轮廓贴纸</option></select></label>
