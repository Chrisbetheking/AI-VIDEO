  ...
  return <main>
    ...
    <section className="grid two">
      ...
      <div className="card">
        ...
        <Field label="抖音分享口令 / 视频 URL（可选）"><input value={sourceUrl} onChange={e => setSourceUrl(e.target.value)} placeholder="可直接粘贴抖音复制链接整段；系统会尽力采集视频，失败则拆解分享文案" /></Field>
    ...
    {extract && <section className="card"><h2>提取结果</h2><p><b>状态：</b>{extract.status}{extract.collector_status ? ` · 采集：${extract.collector_status}` : ''}</p><p>{extract.summary}</p>{extract.collected_video_url && <div className="miniVideo"><p><b>已采集到视频：</b>{extract.collected_video_name}</p><video controls src={`${import.meta.env.VITE_API_BASE || ''}${extract.collected_video_url}`} /></div>}<pre>{extract.transcript || '暂无 transcript'}</pre>{safeExtractWarnings.map(w => <Alert key={w} text={w} />)}</section>}
