...
const modules: { key: ModuleKey; icon: string; title: string; desc: string; tag: string }[] = [
  ...
  { key: 'copy', icon: '文', title: '文案 / 旁白', desc: '按路线生成开场、完整旁白或图文脚本，不强制数字人', tag: '文案' },
...
const workflowSteps: { key: ModuleKey; step: string; title: string; desc: string; action: string }[] = [
  { key: 'copy', step: '01', title: '文案路线', desc: '先选纯素材、数字人开场、真人拍摄或图文路线。', action: '定路线' },
  { key: 'voice', step: '02', title: '配音/旁白', desc: '按当前路线生成开场配音或整条旁白。', action: '配音' },
...
function AppInner() {
  ...
  const [assetLibraryPage, setAssetLibraryPage] = useState<'video' | 'image'>('video')
  ...
  const [oneClickInstruction, setOneClickInstruction] = useState('默认生成一条纯素材混剪获客短视频：先用强钩子旁白开场，再按楼盘/马来西亚风光/学校/配套素材讲清楚。只有选择数字人路线时，才额外生成真人模板口型同步开场。')
  const [oneClickChatInput, setOneClickChatInput] = useState('把开头改得更像老板提醒客户，减少书面词，字幕重点更强。')
  const [oneClickOutputType, setOneClickOutputType] = useState('mixed_video')
  ...
  const [lastHandoff, setLastHandoff] = useState('新流程：先选生产路线；默认纯素材混剪。需要真人感片头时再生成数字人开场，否则直接选素材、配音、合成。')
  ...
  const [v1034Status, setV1034Status] = useState<any>(null)
  const [v1034MaterialCategories, setV1034MaterialCategories] = useState<string[]>([])
  const [v1034MaterialItems, setV1034MaterialItems] = useState<any[]>([])
  const [v1034MaterialMeta, setV1034MaterialMeta] = useState({ category: '生活配套', city: '马来西亚', area: '吉隆坡', source: '自己上传', reusable: true, note: '用于马来西亚房产获客视频' })
  const [v1034AccountPaste, setV1034AccountPaste] = useState('Justin陈皆廷,https://www.douyin.com/,douyin,马来西亚房产')
  const [v1034Accounts, setV1034Accounts] = useState<any[]>([])
  const [v1034AccountResult, setV1034AccountResult] = useState<any>(null)
  const [v1034OpenClawInput, setV1034OpenClawInput] = useState('')
  const [v1034OpenClawJob, setV1034OpenClawJob] = useState<any>(null)
  const [v1034ObsidianTitle, setV1034ObsidianTitle] = useState('马来西亚房产获客结构复盘')
  const [v1034ObsidianContent, setV1034ObsidianContent] = useState('选题：\n账号：\n素材：\n爆款结构：\n下次优化：')
  const [v1034ObsidianNotes, setV1034ObsidianNotes] = useState<any[]>([])
  const [v1034GuardResult, setV1034GuardResult] = useState<any>(null)
  ...
  }, [materialAssets, assetLibraryPage, assetKindFilter, assetFolderFilter, assetSearch, assetSort, assetTimeFilter])
  ...
  const materialVideoCount = useMemo(() => materialAssets.filter(a => a.kind === 'video').length + collectedVideos.length, [materialAssets, collectedVideos])
  const materialImageCount = useMemo(() => materialAssets.filter(a => a.kind === 'image').length, [materialAssets])
  const assetFolderTabOptions = useMemo<[AssetFolderKey, string][]>(() => assetLibraryPage === 'video'
    ? [['all','全部视频'], ['digital_human','数字人片段'], ['self','自己拍的'], ['provided','别人提供'], ['collected','采集视频']]
    : [['all','全部图片'], ['self','自己拍的'], ['provided','别人提供'], ['image','图片素材'], ['ai','AI生成图']], [assetLibraryPage])
  ...
  useEffect(() => { setAssetVisibleCount(24); setCollectedVisibleCount(40) }, [assetLibraryPage, assetKindFilter, assetFolderFilter, assetSearch, assetSort, assetTimeFilter])
...
${selectedAssetScriptContext || '暂未选择素材，先给出通用画面建议。'}`, 3600), [businessPositioning, customerSegments, audience, sellingPointsWithProfile, conversionGoal, privateDomainAssets, reportDelivery, heatRadarForCopy, trendKeywords, memoryContext?.learning_summary, selectedAssetScriptContext])
  ...
  const isSelfShootRoute = normalizedOutputType === 'self_shoot'
  ...
  const leadScore = useMemo(() => {
    ...
    if (isDigitalHumanRoute && digitalHuman?.video_url) score += 8
    if (!isDigitalHumanRoute) score += 5
    if (video?.video_url) score += 15
    return Math.min(100, score)
  }, [extract, copy.hook, voiceSegments.length, selectedMaterialIds.length, video, isDigitalHumanRoute, digitalHuman?.video_url])
  ...
  const pipelineTodos = useMemo(() => [
    ...
    { ok: Boolean(copy.script), text: `生成${productionRouteLabel}脚本，确认是否入知识库`, go: 'copy' as ModuleKey },
    { ok: selectedMaterialIds.length > 0, text: '选择楼盘/风光/配套素材，排好顺序和截取区间', go: 'assets' as ModuleKey },
    ...(isDigitalHumanRoute ? [{ ok: Boolean(digitalHuman?.video_url), text: '生成可选数字人开场片段，或切换为纯素材混剪跳过', go: 'digitalHuman' as ModuleKey }] : []),
    { ok: Boolean(audio), text: '生成最终配音，确认语速和停顿', go: 'voice' as ModuleKey },
  ...
  ], [industry, audience, leadPlan, extract, copy.script, audio, selectedMaterialIds, video, publish, productionRouteLabel, isDigitalHumanRoute, digitalHuman?.video_url])
...
4. 热度雷达只作为选题和痛点参考，不照抄别人。`, 4000),
  ...
  async function generateDigitalHumanIntroCopy() {
    ...
    const routeGoal = isDigitalHumanRoute
      ? `生成 ${introSeconds} 秒数字人真人开场口播：像房产顾问对镜头讲话，最后一句自然切到楼盘/风光素材。`
      : isSelfShootRoute
        ? `生成 ${introSeconds} 秒真人拍摄开场口播：适合顾问自己拿手机拍，句子短，提词器好念。`
        : isGraphicRoute
          ? `生成图文引流首屏标题、前三秒钩子和短说明：适合小红书/朋友圈收藏转发。`
          : `生成 ${introSeconds} 秒纯素材混剪开场旁白：不用真人出镜，用字幕和画面切入痛点。`
    const routeRule = isDigitalHumanRoute
      ? '【数字人规则】只写 5-15 秒开场；不要写完整长视频；不要夸张承诺；最后一句要能接楼盘/风光素材。'
      : isSelfShootRoute
        ? '【真人拍摄规则】句子要适合提词器；给顾问自然口播，不要 AI 腔；可以提示表情/停顿。'
        : isGraphicRoute
          ? '【图文规则】首图标题要强，中间讲重点，最后引导私信领取资料；不要生成数字人口播。'
          : '【纯素材混剪规则】不需要数字人；用旁白 + 画面字幕开场，直接接楼盘、风光、学校和配套素材。'
    const res = await run('生成开场/首段稿', () => apiPost<GeneratedCopy>('/api/generate-copy', {
      topic: routeGoal,
      industry,
      audience,
      selling_points: limitText(`${strategyMemoryContext}
...
【当前路线】${productionRouteLabel}
【路线说明】${productionRouteHint}`, 1200),
      style: `${style}，短视频口语化，像本地顾问真实提醒客户。`,
    ...
    setLastHandoff(isDigitalHumanRoute
      ? '数字人开场稿已生成。下一步先生成配音，再到“可选数字人”用 fal 真人模板口型同步。'
      : isGraphicRoute
        ? '图文首屏/钩子已生成。可以继续生成图文包，或转成旁白配音后做视频。'
        : '开场旁白已生成。可以直接生成配音、选择素材并合成；这条路线不强制数字人。')
    setActive(isGraphicRoute ? 'subtitleCover' : 'voice')
  }

  async function generateScriptFromSelectedAssets() {
    const targetSeconds = safeProjectDuration(selectedAssetEstimatedSeconds || 45)
    const routeBridge = isDigitalHumanRoute
      ? (digitalHuman?.video_url ? '开头已有数字人片段；完整旁白要自然承接片头，后面按素材顺序讲。' : '如果后面补数字人片头，脚本首句要能自然承接；如果不补，也能作为纯素材旁白使用。')
      : isSelfShootRoute
        ? '不使用数字人，按真人顾问拍摄 + B-roll 画面顺序写完整旁白。'
        : isGraphicRoute
          ? '优先生成图文/短视频都能用的分屏文案；不需要数字人口播。'
          : '不使用数字人，直接按楼盘/风光/B-roll 素材顺序写完整旁白。'
...
【当前路线】${productionRouteLabel}
【写法要求】${routeBridge} 按素材画面顺序写；不要照抄同行；不要承诺收益；每 1-2 句适合一段字幕；结尾引导私信领取资料。`, 1700),
    ...
    setCopy(res!)
    setVoiceSegments((res?.script || '').split(/\n+/).map(x => x.trim()).filter(Boolean).slice(0, 12).map(text => ({ ...defaultSegment, text })))
    setLastHandoff(isDigitalHumanRoute
      ? '已按素材总时长生成完整旁白稿。现在生成最终配音，再合成数字人开场 + 素材混剪。'
      : '已按素材总时长生成完整旁白稿。现在生成配音并直接合成素材混剪，不需要数字人。')
    setActive('voice')
  }
  ...
  function skipDigitalHumanAndUseAssets() {
    setOneClickOutputType('mixed_video')
    setLastHandoff('已切换为纯素材混剪路线：不生成数字人，直接选择楼盘/风光/B-roll 素材，按素材时长补全旁白并合成。')
    setActive('assets')
  }
  ...
  async function composeVideo() {
    ...
    const v1034Plan = await apiPost<any>('/api/video/v10-34/sanitize-shot-plan', {
      title: safeText(copy.title, '短视频'),
      script: composeScript,
      keywords: v1034Keywords(),
      banned_words: v1034BannedWords(),
      shots: assetPlan.map((item, index) => ({
        ...item,
        index: index + 1,
        raw_clip: item.filename || item.url,
        prompt: safeText(chosen[index]?.original_name || chosen[index]?.filename || item.filename, ''),
        scene_type: v1034Keywords()[index % Math.max(1, v1034Keywords().length)] || '素材混剪',
        narration_segment: composeSegments[index]?.text || composeScript.split(/\n+/)[index] || copy.hook || copy.title,
        duration: item.kind === 'image' ? item.image_seconds : Math.max(0.8, safeNumber(item.video_end, 0) - safeNumber(item.video_start, 0)),
      }))
    }).catch(() => null)
    const guardedAssetPlan = assetPlan.map((item, index) => ({
      ...item,
      transition: 'smooth_dissolve_no_flash',
      transition_to_next: 'smooth_dissolve_no_flash',
      concat_mode: 'crossfade',
      v10_34_metadata: v1034Plan?.shots?.[index] || {},
    }))
    setV1034GuardResult(v1034Plan)

    const res = await run('合成视频并烧字幕', () => apiPost<ComposeResponse>('/api/compose-video', {
      title: safeText(copy.title, '短视频'),
      script: composeScript,
      asset_ids: chosen.map(a => String(a.id)),
      asset_plan: guardedAssetPlan,
      transition_policy: { safe_transition: 'smooth_dissolve_no_flash', concat: 'crossfade', banned: ['cut', 'smooth_cut', 'flash', 'pull_out'] },
    ...
    setVideo(res!)
    await apiPost('/api/video/v10-34/complete-job', {
      job_id: (res as any)?.job_id || (res as any)?.video_name || `completed_${Date.now()}`,
      title: safeText(copy.title, '短视频'),
      video: res,
      script: composeScript,
      audio_file_name: composeAudio?.file_name || '',
      shots: guardedAssetPlan.map((item, index) => ({
        raw_clip: item.filename || item.url,
        prompt: item.v10_34_metadata?.prompt || chosen[index]?.original_name || chosen[index]?.filename || '',
        negative_prompt: item.v10_34_metadata?.negative_prompt || 'hard cut, flash, pull_out, watermark, logo',
        scene_type: item.v10_34_metadata?.scene_type || '素材混剪',
        narration_segment: item.v10_34_metadata?.narration_segment || composeSegments[index]?.text || '',
        duration: item.v10_34_metadata?.duration || item.image_seconds || 0,
        job_id: (res as any)?.job_id || (res as any)?.video_name || '',
        transition: 'smooth_dissolve_no_flash',
        concat_mode: 'crossfade',
      }))
    }).catch(() => null)
    await refreshV1034Status().catch(() => null)
    setLastHandoff(hasDigitalIntro ? '视频已出片：数字人片头和后续素材已分离；V10.34A 已保存镜头闭环元数据。' : '视频已出片。V10.34A 已强制 smooth_dissolve_no_flash + crossfade，并保存 raw_clip/prompt/scene_type/duration/job_id。')
  }
  ...
  async function refreshV1034Status() {
    const [status, cats, mats, accs, notes] = await Promise.all([
      apiGet<any>('/api/video/ai-console/status').catch(() => null),
      apiGet<any>('/api/video/material-library/categories').catch(() => null),
      apiGet<any>('/api/video/material-library').catch(() => null),
      apiGet<any>('/api/video/account-library/accounts').catch(() => null),
      apiGet<any>('/api/video/obsidian/notes').catch(() => null),
    ])
    if (status) setV1034Status(status)
    if (Array.isArray(cats?.categories)) setV1034MaterialCategories(cats.categories)
    if (Array.isArray(mats?.items)) setV1034MaterialItems(mats.items)
    if (Array.isArray(accs?.items)) setV1034Accounts(accs.items)
    if (Array.isArray(notes?.items)) setV1034ObsidianNotes(notes.items)
    return status
  }

  function v1034Keywords() {
    return subtitleHighlight.split(/[,，、\n]/).map(x => x.trim()).filter(Boolean)
  }

  function v1034BannedWords() {
    return Array.from(new Set([...matchedBadWords, ...['稳赚', '保证收益', '夸张承诺', '无风险']])).filter(Boolean)
  }

  async function v1034SanitizeCurrentShotPlan(showMessage = true) {
    const sourceShots = (copy.shots && copy.shots.length ? copy.shots : selectedMaterialAssets.map(a => safeText(a.original_name || a.filename))).filter(Boolean)
    const shots = sourceShots.map((name, index) => ({
      index: index + 1,
      narration_segment: (voiceSegments[index]?.text || currentScript.split(/\n+/)[index] || copy.hook || copy.title || '').trim(),
      scene_type: v1034Keywords()[index % Math.max(1, v1034Keywords().length)] || '生活配套',
      prompt: name,
      negative_prompt: v1034BannedWords().join(', '),
      duration: segmentSeconds[index] || estimateSeconds(voiceSegments[index]?.text || name),
      transition: segmentTransitions[index] || 'smooth_dissolve_no_flash',
    }))
    const res = await run('V10.34A 检查转场闭环', () => apiPost<any>('/api/video/v10-34/sanitize-shot-plan', {
      title: copy.title || industry,
      script: currentScript,
      keywords: v1034Keywords(),
      banned_words: v1034BannedWords(),
    ...
    setV1034GuardResult(res)
    if (showMessage) setLastHandoff('V10.34A 已检查：禁用 cut / flash / pull_out，统一 smooth_dissolve_no_flash + crossfade，并补齐镜头元数据。')
    return res
  }

  async function v1034GenerateVoiceDraft() {
    const res = await run('V10.34B 生成逐句口播稿', () => apiPost<any>('/api/video/v10-34/step2/script', {
      title: copy.title || industry,
      topic: copy.title || industry,
      audience,
      selling_points: sellingPointsWithProfile,
      script: currentScript,
      keywords: v1034Keywords(),
    ...
    const segments = Array.isArray(res?.segments) ? res.segments.map((seg: any) => ({
      ...defaultSegment,
      text: safeText(seg.text || seg.narration || seg, ''),
      emotion: safeText(seg.emotion, '自然可信'),
      speed_ratio: Number(seg.speed_ratio || 1.05),
      volume_ratio: Number(seg.volume_ratio || 1),
      pitch_ratio: Number(seg.pitch_ratio || 1),
      pause_after_ms: Number(seg.pause_after_ms || 320),
    })).filter((seg: VoiceSegment) => seg.text) : []
    if (segments.length) {
      setVoiceSegments(segments)
      setSegmentSeconds(Object.fromEntries(segments.map((seg: VoiceSegment, idx: number) => [idx, estimateSeconds(seg.text, seg.speed_ratio)])))
    }
    if (res?.script) setCopy(prev => ({ ...prev, title: res.title || prev.title, hook: res.hook || prev.hook, script: res.script }))
    if (Array.isArray(res?.keywords) && res.keywords.length) setSubtitleHighlight(res.keywords.join(','))
    setLastHandoff('V10.34B 已挂回原配音页：逐句口播稿、关键词和禁用词已写入当前项目，可每句试听或整段生成。')
    setActive('voice')
  }

  async function v1034PreviewVoiceSegment(index?: number) {
    const source = typeof index === 'number' ? [voiceSegments[index]].filter(Boolean) : (voiceSegments.length ? voiceSegments : [{ ...defaultSegment, text: currentScript }])
    if (!source.length || !source.some(x => x.text.trim())) { setError('请先生成或填写逐句口播稿。'); return }
    const res = await run(typeof index === 'number' ? `V10.34B 试听第${index + 1}句` : 'V10.34B 试听整段口播', () => apiPost<TTSResponse>('/api/tts-segments', {
      segments: source,
      voice,
      overall_rate: '+0%'
    }))
    setAudio(res!)
    setLastHandoff(typeof index === 'number' ? `第 ${index + 1} 句试听已生成。` : '整段口播试听已生成，后续合成会按真实音频时长校准。')
  }

  async function v1034SaveVoiceVersion() {
    const res = await run('V10.34B 保存口播版本', () => apiPost<any>('/api/video/v10-34/step2/save', {
      title: copy.title || industry,
      hook: copy.hook,
      script: currentScript,
      voice,
    ...
    setLastHandoff(`口播版本已保存：${res?.version_id || 'saved'}。`)
    await refreshV1034Status().catch(() => null)
  }

  async function v1034UploadCategorizedMaterial(e: any) {
    const files: FileList | null = e?.target?.files || null
    if (!files || !files.length) return
    const meta = v1034MaterialMeta
    const missing = [
      !meta.category ? '分类' : '',
      !meta.city.trim() ? '城市' : '',
      !meta.area.trim() ? '区域' : '',
    ...
    if (missing.length) {
      setError(`素材上传已拦截：${missing.join('、')} 必填。`)
      e.target.value = ''
      return
    }
    const res = await run('V10.34C 分类上传素材', () => uploadV1034Material(files, meta))
    setV1034MaterialItems(prev => [...(res?.items || []), ...prev])
    setLastHandoff(`已分类上传 ${res?.items?.length || files.length} 个素材，分类：${meta.category}。`)
    e.target.value = ''
    await reloadAssets().catch(() => null)
    await refreshV1034Status().catch(() => null)
  }

  function v1034ParseAccounts() {
    return v1034AccountPaste.split(/\n+/).map(line => line.trim()).filter(Boolean).map(line => {
      const parts = line.split(/[,，\t]/).map(x => x.trim())
      if (parts.length === 1) return { url: parts[0], platform: 'douyin', nickname: '', note: '' }
      return { nickname: parts[0] || '', url: parts[1] || '', platform: parts[2] || 'douyin', note: parts.slice(3).join('，') }
    }).filter(x => x.url || x.nickname)
  }

  async function v1034ImportAccounts() {
    const accounts = v1034ParseAccounts()
    if (!accounts.length) { setError('请先粘贴账号：昵称,链接,平台,备注。'); return }
    const res = await run('V10.34D 导入账号并分类', () => apiPost<any>('/api/video/account-library/import', { accounts, classify_provider: 'deepseek_then_rule_fallback' }))
    setV1034AccountResult(res)
    await refreshV1034Status()
    setLastHandoff(`账号库已导入 ${res?.count || accounts.length} 个账号，并完成分类评分。`)
  }

  async function v1034StartOpenClawCollect() {
    const urls = v1034OpenClawInput.split(/\n+/).map(x => x.trim()).filter(Boolean)
    if (!urls.length) { setError('请先输入 OpenClaw 要采集的账号/视频链接。'); return }
    const res = await run('V10.34E 启动 OpenClaw 采集', () => apiPost<any>('/api/video/openclaw/collect/start', {
      urls,
      topic: copy.title || industry,
      keywords: v1034Keywords(),
      import_to: ['account_library', 'material_library', 'obsidian'],
    }))
    setV1034OpenClawJob(res?.job || res)
    setLastHandoff(`OpenClaw 采集任务已创建：${res?.job_id || res?.job?.id || 'queued'}。`)
  }

  async function v1034CheckOpenClawJob() {
    const jobId = v1034OpenClawJob?.id || v1034OpenClawJob?.job_id
    if (!jobId) { setError('当前没有 OpenClaw 任务 ID。'); return }
    const res = await run('V10.34E 查询 OpenClaw 任务', () => apiGet<any>(`/api/video/openclaw/collect/job/${encodeURIComponent(jobId)}`))
    setV1034OpenClawJob(res?.job || res)
  }

  async function v1034WriteObsidianNote() {
    const content = `${v1034ObsidianContent}\n\n---\n当前项目：${copy.title || industry}\n关键词：${subtitleHighlight}\n账号数：${v1034Accounts.length}\n素材数：${v1034MaterialItems.length}\n口播段数：${voiceSegments.length}\n`
    const res = await run('V10.34F 写入 Obsidian', () => apiPost<any>('/api/video/obsidian/notes', {
      title: v1034ObsidianTitle,
      content,
      tags: ['AI视频', 'V10.34', industry].filter(Boolean),
    }))
    setLastHandoff(`Obsidian 笔记已写入：${res?.path || res?.title || 'saved'}。`)
    await refreshV1034Status().catch(() => null)
  }

  function renderV1034VideoLoopPanel() {
    return <div className="v1034NativePanel">
      <div className="v1034PanelHead"><div><strong>V10.34A 视频闭环规则</strong><span>挂在原一键生成 / 成片流程里，不改整体 UI。</span></div><Pill tone="green">原生接入</Pill></div>
      <div className="v1034RuleGrid">
        <span>禁用 cut / smooth_cut / flash / pull_out</span><span>强制 smooth_dissolve_no_flash</span><span>拼接 crossfade</span><span>保存 raw_clip / prompt / negative_prompt / scene_type / narration_segment / duration / job_id</span>
      </div>
      <div className="buttonRow"><Button busy={busy === 'V10.34A 检查转场闭环' ? busy : ''} label="检查当前镜头闭环" onClick={() => v1034SanitizeCurrentShotPlan(true)} kind="soft" /><Button label="去成片合成" onClick={() => setActive('video')} kind="ghost" /></div>
      {v1034GuardResult?.shots && <div className="v1034MiniList">{v1034GuardResult.shots.slice(0, 5).map((s: any, i: number) => <div key={i}><b>{i + 1}</b><span>{s.scene_type || 'scene'} · {s.transition_to_next}</span><em>{s.narration_segment || s.prompt}</em></div>)}</div>}
    </div>
  }

  function renderV1034VoicePanel() {
    return <div className="v1034NativePanel">
      <div className="v1034PanelHead"><div><strong>V10.34B 第二步：逐句口播 / 配音试听</strong><span>关键词和禁用词参与生成，结果直接写进原配音分段。</span></div><Pill tone="purple">配音页原生</Pill></div>
      <div className="grid2"><Field label="关键词参与生成"><input value={subtitleHighlight} onChange={e => setSubtitleHighlight(e.target.value)} /></Field><Field label="禁用词 / 风险表达"><input value={v1034BannedWords().join('，')} readOnly /></Field></div>
      <div className="buttonRow"><Button busy={busy === 'V10.34B 生成逐句口播稿' ? busy : ''} label="生成逐句口播稿" onClick={v1034GenerateVoiceDraft} kind="soft" /><Button busy={busy === 'V10.34B 试听整段口播' ? busy : ''} label="试听整段" onClick={() => v1034PreviewVoiceSegment()} kind="ghost" /><Button busy={busy === 'V10.34B 保存口播版本' ? busy : ''} label="保存口播版本" onClick={v1034SaveVoiceVersion} kind="ghost" /></div>
      {voiceSegments.length > 0 && <div className="v1034MiniList compact">{voiceSegments.slice(0, 6).map((seg, i) => <div key={`${seg.text}-${i}`}><b>{i + 1}</b><span>{seg.emotion} · {estimateSeconds(seg.text, seg.speed_ratio).toFixed(1)}s</span><em>{seg.text}</em><button className="mini" onClick={() => v1034PreviewVoiceSegment(i)}>试听本句</button></div>)}</div>}
    </div>
  }

  function renderV1034MaterialPanel() {
    return <div className="v1034NativePanel">
      <div className="v1034PanelHead"><div><strong>V10.34C 素材库分类上传</strong><span>分类、城市、区域、来源、是否复用、备注全部挂在原素材库。</span></div><Pill tone="orange">不填不准传</Pill></div>
      <div className="grid3"><Field label="分类"><select value={v1034MaterialMeta.category} onChange={e => setV1034MaterialMeta({ ...v1034MaterialMeta, category: e.target.value })}>{(v1034MaterialCategories.length ? v1034MaterialCategories : ['生活配套','交通出勤','医疗药房','餐饮食馆','户型采光','学校教育','其他']).map(x => <option key={x}>{x}</option>)}</select></Field><Field label="城市"><input value={v1034MaterialMeta.city} onChange={e => setV1034MaterialMeta({ ...v1034MaterialMeta, city: e.target.value })} /></Field><Field label="区域"><input value={v1034MaterialMeta.area} onChange={e => setV1034MaterialMeta({ ...v1034MaterialMeta, area: e.target.value })} /></Field><Field label="来源"><input value={v1034MaterialMeta.source} onChange={e => setV1034MaterialMeta({ ...v1034MaterialMeta, source: e.target.value })} /></Field><Field label="是否可复用"><select value={v1034MaterialMeta.reusable ? 'true' : 'false'} onChange={e => setV1034MaterialMeta({ ...v1034MaterialMeta, reusable: e.target.value === 'true' })}><option value="true">可复用</option><option value="false">仅本次</option></select></Field><Field label="备注"><input value={v1034MaterialMeta.note} onChange={e => setV1034MaterialMeta({ ...v1034MaterialMeta, note: e.target.value })} /></Field></div>
      <div className="buttonRow"><label className="btn soft fileBtn">拖拽/选择上传<input type="file" multiple onChange={v1034UploadCategorizedMaterial} /></label><Button label="刷新分类素材" onClick={() => refreshV1034Status()} kind="ghost" /></div>
      <div className="v1034MiniList compact">{v1034MaterialItems.slice(0, 6).map((item: any) => <div key={item.id || item.filename}><b>{item.category || '素材'}</b><span>{item.city || '-'} · {item.area || '-'} · {item.reusable ? '可复用' : '仅本次'}</span><em>{item.original_name || item.filename}</em></div>)}{!v1034MaterialItems.length && <Empty>暂无 V10.34 分类素材。</Empty>}</div>
    </div>
  }

  function renderV1034AccountOpenClawObsidianPanel() {
    return <div className="v1034NativePanel">
      <div className="v1034PanelHead"><div><strong>V10.34D/E/F 账号库、OpenClaw、Obsidian</strong><span>全部挂在原账号库页面，不新增单页 UI。</span></div><Pill tone="blue">账号增长</Pill></div>
      <div className="grid2"><Field label="账号导入：每行 昵称,链接,平台,备注"><textarea value={v1034AccountPaste} onChange={e => setV1034AccountPaste(e.target.value)} /></Field><div className="v1034Stack"><Button busy={busy === 'V10.34D 导入账号并分类' ? busy : ''} label="导入账号 + DeepSeek 分类" onClick={v1034ImportAccounts} kind="soft" /><p>已入库 {v1034Accounts.length} 个账号。{v1034AccountResult?.count ? `本次 ${v1034AccountResult.count} 个。` : ''}</p></div></div>
      <div className="grid2"><Field label="OpenClaw 采集链接 / 账号"><textarea value={v1034OpenClawInput} onChange={e => setV1034OpenClawInput(e.target.value)} placeholder="一行一个账号主页或视频链接" /></Field><div className="v1034Stack"><Button busy={busy === 'V10.34E 启动 OpenClaw 采集' ? busy : ''} label="启动采集" onClick={v1034StartOpenClawCollect} kind="soft" /><Button busy={busy === 'V10.34E 查询 OpenClaw 任务' ? busy : ''} label="查询采集状态" onClick={v1034CheckOpenClawJob} kind="ghost" /><p>{v1034OpenClawJob ? `${v1034OpenClawJob.id || v1034OpenClawJob.job_id} · ${v1034OpenClawJob.status || 'queued'}` : '暂无采集任务'}</p></div></div>
      <div className="grid2"><Field label="Obsidian 笔记标题"><input value={v1034ObsidianTitle} onChange={e => setV1034ObsidianTitle(e.target.value)} /></Field><Field label="沉淀内容"><textarea value={v1034ObsidianContent} onChange={e => setV1034ObsidianContent(e.target.value)} /></Field></div>
      <div className="buttonRow"><Button busy={busy === 'V10.34F 写入 Obsidian' ? busy : ''} label="写入 Obsidian 生长笔记" onClick={v1034WriteObsidianNote} kind="soft" /><span className="mutedInline">已有笔记 {v1034ObsidianNotes.length} 条</span></div>
    </div>
  }

  function renderV1034ConsolePanel() {
    return <div className="v1034NativePanel console">
      <div className="v1034PanelHead"><div><strong>V10.34G AI 总控台</strong><span>只做状态总览，仍保留原运营中控台 UI。</span></div><Button label="刷新 V10.34 状态" onClick={() => refreshV1034Status()} kind="ghost" /></div>
      <div className="v1034StatusGrid"><div><span>视频链路</span><strong>{v1034Status?.video_transition_lock?.safe_transition || 'smooth_dissolve_no_flash'}</strong><em>cut/flash/pull_out 已禁用</em></div><div><span>分类素材</span><strong>{v1034Status?.counts?.materials ?? v1034MaterialItems.length}</strong><em>素材库</em></div><div><span>账号库</span><strong>{v1034Status?.counts?.accounts ?? v1034Accounts.length}</strong><em>DeepSeek 分类</em></div><div><span>OpenClaw</span><strong>{v1034Status?.counts?.openclaw_jobs ?? (v1034OpenClawJob ? 1 : 0)}</strong><em>采集任务</em></div><div><span>Obsidian</span><strong>{v1034Status?.counts?.obsidian_notes ?? v1034ObsidianNotes.length}</strong><em>生长笔记</em></div></div>
    </div>
  }

  const stageCards: Array<{ key: ModuleKey; label: string; done: boolean; value: string }> = [
    { key: 'copy', label: '1 路线/文案', done: Boolean(copy.hook || copy.script), value: copy.title || productionRouteLabel },
    { key: 'shooting', label: '2 脚本/拍摄', done: Boolean(shootingPlan) || !isSelfShootRoute, value: isSelfShootRoute ? (shootingPlan?.summary || '上传脚本或生成任务单') : '非必需' },
  ...
  const contentNavKeys: ModuleKey[] = ['copy','voice','digitalHuman','assets','video','subtitleCover','growth']
    ...
    <main className="studioMain">
      <header className="heroHeader">
        <div>
          ...
          <p>默认走纯素材混剪；需要真人感片头时再启用数字人，最后统一成片和复盘投流。</p>
        </div>
      ...
      {moduleVisible('oneClick') && active === 'oneClick' && <section className="card modulePanel oneClickPanel">
        ...
        <div className="grid3">
          <Field label="输出类型"><select value={oneClickOutputType} onChange={e => setOneClickOutputType(e.target.value)}><option value="mixed_video">纯素材混剪（默认）</option><option value="digital_human">数字人开场 + 混剪</option><option value="self_shoot">真人拍摄 + 混剪</option><option value="image_text">图文引流包</option><option value="all">视频 + 图文都要</option></select></Field>
          <Field label="素材方式"><select value={oneClickMaterialMode} onChange={e => setOneClickMaterialMode(e.target.value)}><option value="selected_assets">使用已选素材</option><option value="manual_later">先出方案，素材后补</option><option value="digital_human_only">只做数字人测试</option><option value="ai_image">AI 生成图文素材</option></select></Field>
        ...
        {oneClick && <div className="oneClickResult">
          <div className="resultBox"><h3>{oneClick.project_title}</h3><p>{oneClick.summary}</p><div className="buttonRow"><button className="btn soft" onClick={() => applyOneClickResult(oneClick)}>重新同步到步骤</button><button className="btn ghost" onClick={() => setActive('copy')}>去文案细改</button><button className="btn ghost" onClick={() => setActive('voice')}>去配音</button><button className="btn ghost" onClick={() => setActive(isDigitalHumanRoute ? 'digitalHuman' : 'assets')}>{isDigitalHumanRoute ? '去可选数字人' : '去素材混剪'}</button></div>{oneClick.warnings?.map(w => <div className="warn" key={w}>{w}</div>)}</div>
          <div className="grid2">
      ...
      {moduleVisible('competitor') && active === 'competitor' && <section className="card modulePanel accountLibraryPanel">
        {renderV1034AccountOpenClawObsidianPanel()}
...
https://www.douyin.com/user/..." /></Field>
      ...
      {moduleVisible('assets') && active === 'assets' && <section className="card modulePanel">
        ...
        <div className="assetLibrarySwitch">
          <button className={assetLibraryPage === 'video' ? 'active' : ''} onClick={() => { setAssetLibraryPage('video'); setAssetKindFilter('all'); setAssetFolderFilter('all') }}>
            <strong>第一页：视频素材</strong><span>只展示 MP4/MOV，含数字人片段、楼盘视频、采集视频</span><em>{materialVideoCount} 个</em>
          </button>
          <button className={assetLibraryPage === 'image' ? 'active' : ''} onClick={() => { setAssetLibraryPage('image'); setAssetKindFilter('all'); setAssetFolderFilter('all') }}>
            <strong>第二页：图片素材</strong><span>只展示 JPG/PNG/WEBP，按原文件夹分类</span><em>{materialImageCount} 个</em>
          </button>
        </div>
        <div className="assetToolbar splitMode">
          <input placeholder={assetLibraryPage === 'video' ? '搜索视频文件名' : '搜索图片文件名'} value={assetSearch} onChange={e => setAssetSearch(e.target.value)} />
          <select value={assetFolderFilter} onChange={e => setAssetFolderFilter(e.target.value as AssetFolderKey)}>{assetFolderTabOptions.map(([key,label]) => <option key={key} value={key}>{label}</option>)}</select>
        ...
        <div className={`assetGridWrap ${assetLibraryPage === 'image' ? 'single' : 'grid2'}`}>
          <div>
            <div className="assetPanelHead"><h3>{assetLibraryPage === 'video' ? '视频素材库' : '图片素材库'}</h3><span>已显示 {visibleMaterialAssets.length}/{filteredMaterialAssets.length}；{assetLibraryPage === 'video' ? '视频只用占位卡，点击预览才打开源文件。' : '图片使用懒加载，按原分类筛选。'}</span></div>
            <div className="assetCards">
              {filteredMaterialAssets.length === 0 && <Empty>{assetLibraryPage === 'video' ? '当前视频页没有匹配素材。可以切换分类、上传 MP4/MOV，或去第二页看图片。' : '当前图片页没有匹配素材。可以切换分类、上传 JPG/PNG/WEBP，或回第一页看视频。'}</Empty>}
            ...
            {visibleMaterialAssets.length < filteredMaterialAssets.length && <div className="loadMoreRow"><button className="btn soft" onClick={() => setAssetVisibleCount(v => v + 24)}>再加载 24 个{assetLibraryPage === 'video' ? '视频' : '图片'}</button></div>}
          </div>
          {assetLibraryPage === 'video' && <div>
      ...
      {moduleVisible('shooting') && active === 'shooting' && <section className="card modulePanel shootingScriptPanel">
        ...
        <div className="productionRouteGrid compact">
          {[['mixed_video','纯素材混剪'],['digital_human','可选数字人'],['self_shoot','自己拍摄'],['image_text','图文窗口']].map(([value,label]) => <button key={value} className={oneClickOutputType === value ? 'selected' : ''} onClick={() => setOneClickOutputType(value)}>{label}</button>)}
        </div>
