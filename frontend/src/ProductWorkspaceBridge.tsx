...
import { clearStoredToken, emptyProjectDraft, getStoredToken, maskToken, ProjectDraft, setStoredToken, WorkspaceTab } from './aiVideoApi'

const DRAFT_KEY = 'ai_video_engineering_project_draft_v13'
...
function DigitalHumanPanel({ project, setProject, goTab }: { project: ProjectDraft; setProject: (p: ProjectDraft) => void; goTab: (tab: WorkspaceTab) => void }) {
  return (
    <section className="ux-card">
      <div className="ux-hero">
        ...
        <span className="ux-badge green">内部自用素材</span>
      </div>
      <div className="ux-grid three">
        <div className="ux-mini"><b>1. 文稿前置</b><span>{project.script ? '已有文稿' : '先去纯 AI 生成文稿'}</span></div>
        <div className="ux-mini"><b>2. 形象素材</b><span>上传照片/口播视频，作为内部素材源</span></div>
        <div className="ux-mini"><b>3. 剪辑承接</b><span>数字人片段进入素材序列，不单独孤立</span></div>
      </div>
      <div className="ux-grid two">
        <label>数字人角色备注<input value={project.digitalHumanRole || ''} onChange={(e) => setProject({ ...project, digitalHumanRole: e.target.value })} placeholder="例如：房产顾问口播" /></label>
        <label>生成模式<select value={project.digitalHumanMode || 'internal_clip'} onChange={(e) => setProject({ ...project, digitalHumanMode: e.target.value })}><option value="internal_clip">内部素材片段</option><option value="placeholder">先生成占位片段</option><option value="none">本条视频不用数字人</option></select></label>
      </div>
      {!project.script && <div className="ux-note">没有文稿时不生成数字人片段。先到「纯 AI 生成」把文稿和分镜出来。</div>}
      <div className="ux-button-row"><button className="ux-primary" onClick={() => goTab('pureai')}>去生成文稿/分镜</button><button className="ux-ghost" disabled={!project.script}>把数字人加入剪辑素材</button></div>
    </section>
  )
}
    ...
    <div className="ux-overlay">
      <div className="ux-shell">
        <header className="ux-topbar">
          <div><p>AI-VIDEO 工程化工作台</p><h1>{tabTitle(tab)}</h1></div>
        ...
        {showToken && <div className="ux-token-box"><label>AI-VIDEO API Token<input value={token} onChange={(e) => setToken(e.target.value)} placeholder="粘贴后台 Token" /></label><button onClick={() => { setStoredToken(token); setToken(getStoredToken()); setShowToken(false) }}>保存 Token</button><button onClick={() => { clearStoredToken(); setToken('') }}>清空</button></div>}
