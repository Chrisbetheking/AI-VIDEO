...
async def _v10_27b_call_original_start_with_model(old_start, payload):
    ...
    try:
        ...
        return {'ok': False, 'provider': 'full_ai_tts_first_semantic_direct_render_v10_27cc', 'error': 'START_MODEL_BIND_FAILED', 'detail': str(exc), 'payload_keys': sorted(list((payload or {}).keys()))}
    try:
        ...
        try:
            ...
            return {'ok': False, 'provider': 'full_ai_tts_first_semantic_direct_render_v10_27cc', 'error': 'OLD_START_MODEL_CALL_FAILED', 'detail': str(exc2), 'first_detail': str(exc), 'payload_keys': sorted(list((payload or {}).keys()))}
    except Exception as exc:
        return {'ok': False, 'provider': 'full_ai_tts_first_semantic_direct_render_v10_27cc', 'error': 'OLD_START_MODEL_CALL_FAILED', 'detail': str(exc), 'payload_keys': sorted(list((payload or {}).keys()))}
...
async def _v10_26_call_old_start(request, payload):
    ...
    if endpoint is None:
        return JSONResponse(status_code=500, content={'ok':False,'provider':'full_ai_tts_first_semantic_direct_render_v10_27cc','error':'OLD_START_ENDPOINT_MISSING'})
    body=json.dumps(payload, ensure_ascii=False).encode('utf-8')
    async def receive():
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={'ok':False,'provider':'full_ai_tts_first_semantic_direct_render_v10_27cc','error':'OLD_START_CALL_FAILED','detail':str(exc),'normalized_payload_keys':sorted(list(payload.keys()))})
...
_V10_27_VERSION = 'v10.27c-direct-original-start'
_V10_27_PROVIDER = 'full_ai_tts_first_semantic_direct_render_v10_27cc'
...
async def _v10_27c_call_true_original_start(request, payload):
    ...
    import inspect
    try:
        from fastapi.responses import JSONResponse as _V10_27C_JSONResponse
    except Exception:
        _V10_27C_JSONResponse = None

    provider = 'full_ai_tts_first_semantic_direct_render_v10_27c'
    payload = payload if isinstance(payload, dict) else {}
    ...
    try:
        ...
        if _V10_27C_JSONResponse:
            return _V10_27C_JSONResponse(status_code=422, content=content)
        return content
    ...
    if _V10_27C_JSONResponse:
        return _V10_27C_JSONResponse(status_code=500, content=content)
    return content
...
async def _v10_27_start(request: _V10_27_Request):
    ...
    payload['demand_acceptance']=preview.get('acceptance'); payload['demand_acceptance_lock']='v10_27c'; payload['generation_allowed']=True
    return await _v10_27c_call_true_original_start(request, payload)
