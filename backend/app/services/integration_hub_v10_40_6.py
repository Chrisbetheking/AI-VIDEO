...
VERSION = "10.40.6"
MODE = "full_chain_closed_loop"
BASE = Path(os.getenv("AI_VIDEO_BASE", "/opt/ai-video"))
STORAGE = BASE / "storage"
ROOT = Path(os.getenv("AI_VIDEO_INTEGRATION_ROOT", str(STORAGE / "integration_hub_v10_40_6")))
...
def install_integration_hub_v10_40_6(app: FastAPI) -> None:
    @app.get("/api/video/integration/health")
    def health():
        vault = vault_path(); return {"ok":True,"version":VERSION,"mode":MODE,"knowledge_backend":True,"obsidian_configured":bool(vault),"obsidian_vault":str(vault) if vault else "","openclaw_orchestration":True,"asset_cleanliness_gate":True,"video_visual_integrity_gate":True,"graphic_window_embedded_tab":True,"button_contracts":True,"storage":str(ROOT)}
    ...
    def obsidian_status():
        vault=vault_path(); return {"ok":True,"version":VERSION,"configured":bool(vault),"vault":str(vault) if vault else "","git":bool(vault and (vault/".git").exists()),"markdown_files":len(list(vault.rglob("*.md"))) if vault else 0}
    ...
    async def openclaw_start(payload: Dict[str, Any] = Body(default_factory=dict)):
        ...
        request={**payload,"source":text(payload.get("source") or "integration_hub_v10_40_6"),"run_openclaw_analysis":True,"persist_results":True}
