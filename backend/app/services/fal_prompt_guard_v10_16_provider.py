...
router = APIRouter(prefix="/api/video/fal-prompt-guard-v10-16", tags=["fal-prompt-guard-v10-16"])
_INSTALLED = False
...
def _rewrite_args(arguments: Any) -> Any:
    ...
    try:
        print("V10_16_FAL_FINAL_ARGUMENTS=" + json.dumps(args, ensure_ascii=False)[:3000], flush=True)
    except Exception:
        pass
    return args


def install_fal_prompt_guard_v10_16(app: FastAPI | None = None) -> None:
    global _INSTALLED
    if _INSTALLED:
        return
    try:
        ...
        setattr(fal_client, "_ai_video_v10_16_prompt_guard", True)
        _INSTALLED = True
        print("V10_16_FAL_PROMPT_GUARD_INSTALLED", flush=True)
    except Exception as exc:
        print("V10_16_FAL_PROMPT_GUARD_INSTALL_FAILED", exc, flush=True)
    if app is not None:
        try:
...
def health() -> dict[str, Any]:
    return {
        "ok": True,
        "provider": "fal_prompt_guard_v10_16",
